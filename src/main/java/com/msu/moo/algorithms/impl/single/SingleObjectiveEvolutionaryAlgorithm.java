...
public class SingleObjectiveEvolutionaryAlgorithm<V extends IVariable, P extends IProblem<V>>
	...
	protected SolutionSet<ISolution<V>, V> population = null;
	...
	public SolutionSet<ISolution<V>, V> initialize(P problem, IEvaluator evaluator, MyRandom rand) {
	...
	public SolutionSet<ISolution<V>, V> next(P problem, IEvaluator evaluator, MyRandom rand, SolutionSet<ISolution<V>, V> population) {
		
		// mating with random selection of the best 20 percent
		SolutionSet<ISolution<V>, V> offsprings = new SolutionSet<>(populationSize);

		// selects per default always the maximal value
		//ASelection<V> selector = new SingleObjectiveBinaryTournament<>(population, rand);
		ISelection<ISolution<V>, V> selector = new RandomSelection<ISolution<V>,V>();

		while (offsprings.size() < populationSize) {
			
			// crossover
			SolutionSet<ISolution<V>, V> selected = selector.next(population, 2, rand);
		...
		SolutionSet<ISolution<V>, V> next = new SolutionSet<>(new HashSet<>(population));
		
		// truncate the population -> survival of the fittest
		next.sort(new SingleObjectiveComparator());
		
		next = new SolutionSet<ISolution<V>, V>(next.subList(0, Math.min(next.size(), populationSize)));
	...
	public SolutionSet<ISolution<V>, V> getPopulation() {
		return population;
	}
	
	
	public static <V> void print(SolutionSet<ISolution<V>, V> population, int n) {
