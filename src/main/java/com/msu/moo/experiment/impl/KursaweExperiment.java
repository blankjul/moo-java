...
public class KursaweExperiment extends AExperiment<NonDominatedSolutionSet<DoubleListVariable>, DoubleListVariable, Kursawe> {
	...
	protected void setAlgorithms(Kursawe problem,
		...
		nsgaII.set("probMutation", 0.3).set("populationSize", 50).set("factory", new DoubleListVariableFactory(range))
				.set("crossover", new SimulatedBinaryCrossover(range)).set("mutation", new RealMutation(range));
